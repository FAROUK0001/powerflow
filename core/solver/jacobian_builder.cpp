        ...
        for (int ptr = y_rows[i]; ptr < y_rows[i + 1]; ptr++) {
            ...
            for (int p_i = 0; p_i < 3; p_i++) {
                for (int p_k = 0; p_k < 3; p_k++) {
                    ...
                    if (i == k && p_i == p_k) {
                        // DIAGONAL
                        std::complex<double> V_i = V[i][p_i];
                        std::complex<double> Y_ii = Y_block(p_i, p_i);
                        std::complex<double> S_i = S_calc[i][p_i];

                        // 1. Angle Derivative
                        dS_dTheta = J_MATH * S_i - J_MATH * V_i * std::conj(Y_ii * V_i);

                        // 2. Magnitude Derivative
                        double mag_V = std::abs(V_i);
                        std::complex<double> V_dir = V_i / mag_V;
                        dS_dVmag = V_dir * std::conj(S_i / V_i) + V_i * std::conj(Y_ii * V_dir);
                    }
                    else {
                        // OFF-DIAGONAL
                        std::complex<double> V_i = V[i][p_i];
                        std::complex<double> V_k = V[k][p_k];
                        std::complex<double> Y_ik = Y_block(p_i, p_k);

                        // 1. Angle Derivative
                        dS_dTheta = -J_MATH * V_i * std::conj(Y_ik * V_k);

                        // 2. Magnitude Derivative
                        double mag_V_k = std::abs(V_k);
                        std::complex<double> V_k_dir = V_k / mag_V_k;
                        dS_dVmag = V_i * std::conj(Y_ik * V_k_dir);
                    }

                    // Separate the complex derivative into Real (dP) and Imaginary (dQ)
                    double dP_dTheta = dS_dTheta.real();
                    double dQ_dTheta = dS_dTheta.imag();

                    double dP_dVmag = dS_dVmag.real();
                    double dQ_dVmag = dS_dVmag.imag();

                    // Calculate matrix indices
                    int row_P = (i - 1) * 6 + (p_i * 2);
                    int row_Q = row_P + 1;

                    int col_Theta = (k - 1) * 6 + (p_k * 2);
                    int col_Vmag  = col_Theta + 1; // <-- MAGNITUDES GO IN ODD COLUMNS
