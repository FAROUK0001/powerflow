...
SolverResult NewtonRaphson::solve(
    ...
    const int num_nodes = node_to_index.size();
    ...
    for (const auto& [node_name, load] : spot_loads) {
        ...
        const int i = node_to_index.at(node_name);
    ...
    for (const auto& d_load : dist_loads) {
        ...
        const int node_a = node_to_index.at(d_load.node_a);
        const int node_b = node_to_index.at(d_load.node_b);
